...
-- Add venture scoping columns
ALTER TABLE public.founder_brain_memory
  ADD COLUMN IF NOT EXISTS snapshot_id uuid REFERENCES public.venture_snapshots(id) ON DELETE CASCADE;
ALTER TABLE public.founder_brain_notes
  ADD COLUMN IF NOT EXISTS snapshot_id uuid REFERENCES public.venture_snapshots(id) ON DELETE CASCADE;
ALTER TABLE public.founder_brain_messages
  ADD COLUMN IF NOT EXISTS snapshot_id uuid REFERENCES public.venture_snapshots(id) ON DELETE CASCADE;
ALTER TABLE public.brain_indexing_jobs
  ADD COLUMN IF NOT EXISTS snapshot_id uuid REFERENCES public.venture_snapshots(id) ON DELETE CASCADE;

CREATE INDEX IF NOT EXISTS founder_brain_memory_snapshot_idx
  ON public.founder_brain_memory (user_id, snapshot_id, kind);
CREATE INDEX IF NOT EXISTS founder_brain_notes_snapshot_idx
  ON public.founder_brain_notes (user_id, snapshot_id, created_at DESC);
CREATE INDEX IF NOT EXISTS founder_brain_messages_snapshot_idx
  ON public.founder_brain_messages (user_id, snapshot_id, created_at);
CREATE INDEX IF NOT EXISTS brain_indexing_jobs_snapshot_idx
  ON public.brain_indexing_jobs (user_id, snapshot_id, created_at DESC);

-- Venture-scoped similarity search. Passing NULL for _snapshot_id returns
-- unassigned/global rows (legacy behavior); a non-null value restricts to
-- rows tagged to that venture.
...
AS $$
  SELECT m.id, m.kind, m.source_ref, m.title, m.content,
   ...
   WHERE m.user_id = _user_id
     ...
     AND (_snapshot_id IS NULL OR m.snapshot_id = _snapshot_id OR m.snapshot_id IS NULL)
...
-- Venture-scoped purge. When _snapshot_id is NULL, purges everything for the
-- user (previous behavior). When set, purges only that venture's brain rows
-- and the venture-scoped deliverables would be untouched (deliverables have
-- no snapshot column today, so we only touch memory/notes/messages/jobs).
