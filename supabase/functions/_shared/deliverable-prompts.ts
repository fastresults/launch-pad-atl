...
6) **Imagery spec** — restate the Section 4b Imagery Plan in full: every route, every section slot, its visual type, aspect ratio, treatment, alt text and generation prompt. Each prompt is 40–80 words, references the mood board + 2 brand hex colors, and names a style ("editorial photography", "isometric illustration", "abstract gradient mesh", "3D clay render", "duotone documentary photo", etc.) plus the aspect ratio (16:9 hero, 4:5 supporting, 1.91:1 OG). State the hard rule for the builder: no section ships text-only, and no page may have two consecutive text-only sections. Tell the builder to generate every image via the platform's image-generation tool at first run, save originals to \`src/assets/\`, and reference them via ES6 imports. Include alt-text for every image.
