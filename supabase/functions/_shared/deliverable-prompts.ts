...
- **Visuals per section**: EVERY section listed above gets at least one visual — photograph, founder/creator portrait, product shot, UI screenshot/mock, diagram, data visual, or an on-brand texture/gradient band. Never allow two consecutive text-only sections. Name the slot, its aspect ratio, and its treatment (full-bleed, inset, portrait card, background wash). The home route carries at least 8 visual slots and every interior route at least 4. For each slot also write its **caption / on-page copy** (the words that print beside the image) and the body sentence it illustrates, and that sentence must appear in this route's copy. Any hero or full-bleed band in this route names its parallax treatment here — background plate 0.25x scroll, midground subject 0.6x, foreground type 1.0x, collapsing to a static composition under \`prefers-reduced-motion\` — and its scrim direction; a hero described without its parallax treatment is a rejected route spec.
...
3) **Design system rules** — restate the SURFACE SYSTEM CONTRACT ladder in full (\`page\`, \`surface\`, \`surface-raised\`, \`surface-inverted\`, \`overlay\`, each with background / foreground / muted-foreground / border tokens for BOTH themes) and this rule verbatim: "Foreground always travels with its surface — any element that changes its background also sets the paired foreground, muted-foreground and border. No component inherits text colour across a surface boundary." Then: light + dark theme parity with no token defined for one theme only, semantic tokens ONLY (no hardcoded \`text-white\`, \`bg-black\`, or \`bg-[#hex]\`), focus rings on every interactive, AA contrast minimum (4.5:1 body, 3:1 large display), typographic scale with line-heights (display 64/72, h1 48/56, h2 36/44, h3 28/36, h4 22/30, body 16/26, caption 13/20). Then restate the LAYOUT & INTERACTION CONTRACT verbatim for the builder, because these are the rules that break in code: the \`Container\` primitive (1280-px max width, gutters 24 px at 360, 32 px at 768, 48 px from 1280 up) wrapping the content of EVERY band including header, announcement bar, footer, cookie banner and 404 — full-bleed backgrounds yes, full-bleed content never, nothing touching the viewport edge; \`overflow-x: hidden\` on the shell and no route scrolling horizontally at 360 / 768 / 1280 / 1920 px; the button system (primary filled in the accent with its paired foreground, secondary outlined, tertiary underlined text with an arrow, 44-px minimum hit target, 12–16 px vertical and 24–32 px horizontal padding, declared hover / active / focus-visible / disabled states, 12-px minimum gap between adjacent CTAs with the primary first) and the rule that a call to action rendered as bare text is a build failure; navigation where the active route indicator (weight change plus a 2-px accent underline or a filled pill, 4.5:1 minimum against the header surface) and \`focus-visible\` are separate treatments and the focus ring is never the active marker; the z-index ladder (base 0, sticky header 50, dropdown 100, overlay 200, modal 300, toast 400) with every overlay — announcement bar, cookie banner, mobile nav, dropdown, modal — carrying its own opaque or backdrop-blurred surface and foreground pair, dismissible, and never covering a headline, caption or primary CTA; the vertical rhythm scale (64 / 96 / 128 px major, 32 / 48 px minor) as the only source of section spacing; no two-column section shipping an empty column; every route deploying the brand accent in at least the primary CTA plus one band or editorial accent, with eyebrows as tracked micro-caps (11–13 px, 0.12–0.16em) rather than body text; and pricing tiers as cards with a price or an explicit pricing basis, full-sentence inclusions and a CTA button per tier.
