...
function useBrandFonts(families: string[]) {
  ...
  useEffect(() => {
    ...
    const id = `share-brand-fonts-${btoa(unescape(encodeURIComponent(key))).replace(/=/g, "")}`;
...
/**
 * The venture's visual brand, shown to the public reader: logo lockups on light
 * and dark, a copyable palette, live font specimens, the mood board, brand DNA
 * and voice, and the calls to action the brand actually uses.
 */
export function ShareBrandBoard({ board }: { board: BrandBoard }) {
  ...
  return (
    <div className="mb-12 space-y-10">
      ...
      {!!board.moodboard?.length && (
        <Block title="Mood board">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {board.moodboard.map((m, i) => (
              <figure key={`${m.url}-${i}`} className="overflow-hidden rounded-2xl border border-border/60 bg-muted/10">
                <img
                  src={m.url}
      ...
      {(dna?.positioning || !!dna?.traits?.length || !!dna?.toneWords?.length) && (
