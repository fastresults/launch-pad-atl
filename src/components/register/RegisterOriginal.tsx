...
const FormSchema = z.object({
  ...
  business_idea: z.string().trim().min(10, "A sentence or two helps us tailor the day").max(2000),
...
export function RegisterOriginal() {
  ...
  const { data: availability } = useQuery({
    ...
    queryFn: () => getCohortAvailability({ cohort_id: cohortId }),
        ...
        <div className="mx-auto max-w-6xl px-6">
          <div className="mb-7 max-w-2xl md:mb-8">
            ...
            <h2 className="text-3xl font-semibold leading-tight tracking-tight md:text-4xl lg:text-5xl">
              Every deliverable.{" "}
              <span className="text-muted-foreground">What it would cost. What it would take.</span>
            </h2>
                  ...
                  {availability.founders.soldOut ? (
                    <>
                      ...
                      <span className="text-foreground">
                        {pubCohortLeft} of {pubCohortCap} Cohort seats left at{" "}
                        {formatPriceCents(selectedCohort.cohortPriceCents)}
                      </span>
                      .
                    </>
                  ) : (
