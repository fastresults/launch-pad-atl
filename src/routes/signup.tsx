...
export default function SignupPage() {
  ...
  useEffect(() => {
    if (loading) return;
    if (isAuthenticated) {
      navigate(isAdmin ? "/admin" : isApprovedMember ? "/dashboard" : "/welcome", { replace: true });
    }
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const { error } = await supabase.auth.signUp({
      email, password,
      options: { emailRedirectTo: window.location.origin + "/login", data: { display_name: name } },
    });
    setSubmitting(false);
      ...
      <div className="flex min-h-screen items-center justify-center bg-background px-4">
        <div className="max-w-md rounded-2xl border border-white/10 bg-card p-8 text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            We sent a confirmation link to <span className="text-foreground">{email}</span>. Click it to finish creating your account.
          </p>
          <Link to="/login" className="mt-6 inline-block text-sm underline">Back to sign in</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="w-full max-w-sm space-y-6 rounded-2xl border border-white/10 bg-card p-8">
        <div className="text-center">
          <h1 className="public-heading">Create account</h1>
