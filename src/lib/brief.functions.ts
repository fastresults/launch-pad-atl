...
export async function getMyBrief() {
  const { data } = await supabase.from("member_briefs").select("*").eq("user_id", await uid()).maybeSingle();
  return data ?? {};
}
export async function updateBriefField(data: { key: BriefKey; value: string }) {
  const userId = await uid();
  const { error } = await supabase.from("member_briefs").upsert({ user_id: userId, [data.key]: data.value }, { onConflict: "user_id" });
  if (error) throw new Error(error.message);
}
export async function adminGetBrief(data: { userId: string }) {
  const { data: row } = await supabase.from("member_briefs").select("*").eq("user_id", data.userId).maybeSingle();
  return row ?? {};
}
